...
In a usual React flow of data, props are used by components to speak to one another. The issue here is when you need to pass props down multiple levels to a grandchild component. In this case, you may just be passing props down to the intermediaries just to get those props down to the grandchild. This is called props drilling. Context API is a solution to this issue where props are only passed down to components that actually need them. This can help avoid errors in code and also make state management much more manageable.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Actions - an object that requires a type and an optional payload property. A type is a string that explains what action is going on. Payload is data that is associated with that action. These actions are passed into a reducer function via a term called "dispatch".
Reducers - a 'pure' function (no side effects) that takes an action and updates state depending on the action type and associated optional payload.
Store - an object that holds all of that state changes within a redux application. Since it holds all changes of state, it is referred to as a 'single source of truth'

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?

Redux-thunk allows us to embed a inner function inside of an outside function. That outside function can return that inside function at a later time. It's essentially a function that returns another function. This allows us to use dispatch, which calls the action creator. It's often used for synchronous API calls.

4. What is your favorite state management system you've learned and this sprint? Please explain why!

I like the reducer pattern because I absolutely hate prop drilling and my OCD is calmed when I am able to clearly define my data and associated actions. Having a single source of truth in a reducer feels really really good.
